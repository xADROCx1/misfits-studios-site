...
function relatedCards(current, all) {
  ...
  <section class="mt-16">
    ...
    <h2 class="font-headline text-3xl md:text-4xl font-black uppercase italic mb-6">You_Might_Also_Like</h2>
...
<main class="max-w-5xl mx-auto px-6 py-12">
  ...
  <section class="hero-card mt-4">
    ...
    <h1 class="font-headline text-5xl md:text-7xl font-black uppercase italic mb-3">${esc(p.name)}</h1>
    <p class="text-lg md:text-xl text-paper/90 mb-5">${esc(p.tagline || '')}</p>
    <div class="mb-5">
      <span class="chip chip-accent">v${esc(p.version)}</span>
      <span class="chip chip-tertiary">${esc(p.category || 'plugin')}</span>
